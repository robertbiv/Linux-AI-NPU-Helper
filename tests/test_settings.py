...
class TestSettingsManager:
    ...
    def test_listener_error_logging(self, tmp_path, caplog):
        import logging
        sm = SettingsManager(path=None)
        sm.add_listener(lambda k, v: (_ for _ in ()).throw(RuntimeError("test warning log")))
        with caplog.at_level(logging.WARNING):
            sm.set("backend", "openai", save=False)
        assert "Settings listener raised an error: test warning log" in caplog.text
